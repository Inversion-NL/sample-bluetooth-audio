...
public class A2DPSinkActivity extends Activity {
            ...
            if (intent.getAction().equals(A2dpSinkHelper.ACTION_CONNECTION_STATE_CHANGED)) {
                ...
                if (device != null) {
                    ...
                    if (newState == BluetoothProfile.STATE_CONNECTED) {
                        speak("Connected to " + deviceName, null);
                    } else if (newState == BluetoothProfile.STATE_DISCONNECTED) {
                        speak("Disconnected from " + deviceName, null);
                    }
                }
            }
        }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Currently not working. Haven't figured out a way to control
        // the incoming bluetooth volume
        ...
        streamMaxVolume = audioManager.getStreamMaxVolume(audioStream);
        Log.d("TextToSpeech", "Max music stream volume is: " + streamMaxVolume);
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_S:
                speak("Sem, ga eens slapen", new Locale("nl_NL"));
        }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQUEST_CODE_ENABLE_DISCOVERABLE) {
            ...
            speak("Bluetooth audio sink is discoverable for " + DISCOVERABLE_TIMEOUT_MS +
                    " milliseconds. Look for a device named " + ADAPTER_FRIENDLY_NAME, null);
    ...
    private void disconnectConnectedDevices() {
        ...
        speak("Disconnecting devices", null);
    ...
    private void speak(String utterance, @Nullable Locale language) {
        if (language == null) language = Locale.US;
        Log.i(TAG, "Speak: " + utterance);
        if (mTtsEngine != null) {

            if (isMusicActive()) Log.d("A2DP", "Music is active");

            else Log.d("A2DP", "No music active");
            streamCurrentVolume = audioManager.getStreamVolume(audioStream);
            Log.d("TextToSpeech", "Current music stream volume is: " + streamCurrentVolume);

            int newVolume = (int) (streamMaxVolume * 0.1);
            audioManager.setStreamVolume(AudioManager.STREAM_RING, newVolume, 0);
            Log.d("TextToSpeech", "Set current music stream volume to: " + newVolume);

            mTtsEngine.setLanguage(language);
    ...
    private void setOnUtteranceProgressListener() {
        if (mTtsEngine != null) {
            Log.d("TextToSpeech", "Register utterance progress listener");
            mTtsEngine.setOnUtteranceProgressListener(new UtteranceProgressListener() {
                ...
                public void onDone(String utteranceId) {
                    Log.d("TextToSpeech", "Done speaking, restoring volume to previous value of " + streamCurrentVolume);
                    audioManager.setStreamVolume(AudioManager.STREAM_RING, streamCurrentVolume, 0);
                }
            ...
            Log.e(TAG, "TTS is uninitialized");
        }
    }
